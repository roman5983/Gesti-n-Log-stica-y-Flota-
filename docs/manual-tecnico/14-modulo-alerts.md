...
7. **Diez hallazgos concretos:**
   ...
   | 4 | 🔴 **`GET_LOCK` está DENTRO de la transacción y `RELEASE_LOCK` se ejecuta antes del `COMMIT`**, dejando una ventana en la que otra evaluación puede entrar y, bajo REPEATABLE READ, no ver las inserciones sin confirmar → **alertas duplicadas**. La corrección no es trivial: el bloqueo es por conexión y Prisma no permite fijarla. *(Sigue sin corregirse; el polling de §14.12 la ejercita con más frecuencia, sin agravarla.)* | Media |
   | 5 | 🔴 **No hay registro de cuándo se evaluó por última vez** (la auditoría es condicional). Sin tareas programadas, nadie puede saber si el tablero está al día — y un tablero vacío por falta de evaluación parece un sistema sano. *(§14.12 mitiga el síntoma —la pantalla se refresca sola cada 60s mientras está abierta— pero no agrega el registro de "última evaluación" que el hallazgo pide.)* | Media |
...
**Por qué se apoyó en el `GET_LOCK` existente en vez de agregar deduplicación en el cliente.** Con varias pestañas de administrador abiertas (o una sola pestaña más el clic manual), el intervalo de 60 segundos puede coincidir con otra evaluación en curso. El diseño ya contemplaba esto — el comentario original de `EVALUATE_LOCK` (línea ~217, citado en §14.4.5) dice explícitamente que sirve para que "dos evaluaciones concurrentes (o un doble clic) no puedan generar alertas duplicadas". El polling es, para el motor, exactamente un doble clic más frecuente: la segunda llamada recibe `ConflictError` (409) del `GET_LOCK` ocupado, y el `catch` del polling lo descata sin reintentar — hay un próximo tick en 60 segundos.

⚠️ **Con el matiz que §14.4.5 ya dejó registrado: el `GET_LOCK` no es 100% hermético.** El hallazgo 4 de §14.9 documenta una ventana real (de microsegundos, entre el `RELEASE_LOCK` y el `COMMIT` de la transacción) en la que dos evaluaciones *pueden*, en el peor caso, insertar las mismas alertas por duplicado bajo `REPEATABLE READ`. Esa ventana **ya existía** antes de este cambio — el botón manual siempre pudo dispararla con dos clics rápidos en dos pestañas—. El polling **no la crea, pero la ejercita con más frecuencia**: pasar de "cada clic humano" a "cada 60 segundos, multiplicado por cada pestaña de administrador abierta" aumenta el número de intentos de `evaluate()` por hora, y por lo tanto la probabilidad acumulada — todavía baja— de que dos de esos intentos caigan exactamente en la ventana de microsegundos. No se implementó la corrección propuesta en §14.4.5 (sacar el `GET_LOCK` fuera de la transacción) como parte de este cambio: es una corrección al motor de evaluación, de alcance mayor al pedido, y queda igual de pendiente que antes — solo que ahora con una razón adicional, aunque menor, para priorizarla.

**Por qué no un job real de backend.** Se consideró y se descartó por alcance: el pedido original describía el estado *actual* como "sin job/polling/websockets" — es decir, nombraba la ausencia como lo que había que resolver, no prescribía la arquitectura de la solución. Agregar un scheduler de servidor (`node-cron`, un worker separado, un cron de sistema operativo que pegue al endpoint) es una opción legítima y probablemente más correcta a largo plazo —no depende de que un administrador tenga una pestaña abierta—, pero exige decidir infraestructura nueva (¿proceso separado o embebido en el mismo servidor Express? ¿qué pasa si el job se cae?) que no estaba pedida ni evaluada. Queda anotada como la alternativa "correcta" si el proyecto crece: el motor de evaluación de este capítulo ya es apto para eso sin cambios — `evaluate()` no sabe ni le importa si lo llama un humano, un `setInterval` del navegador o un cron de servidor.
