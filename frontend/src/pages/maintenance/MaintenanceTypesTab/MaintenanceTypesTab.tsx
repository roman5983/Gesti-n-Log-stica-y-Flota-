...
import { MaintenanceTypeFormDialog } from '@/pages/maintenance/MaintenanceTypeFormDialog/MaintenanceTypeFormDialog';
import type { MaintenanceTypesTabProps } from './MaintenanceTypesTab.types';

/** CRUD of maintenance types (admin). Rendered inside the Maintenance tabs. */
export function MaintenanceTypesTab({ canManage }: MaintenanceTypesTabProps) {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      notify.success(`Tipo "${toDelete.name}" eliminado`);
      setToDelete(null);
      await reload();
    } catch (err) {
  ...
  return (
    <Box>
      ...
      <MaintenanceTypeFormDialog
        ...
        onSaved={() => { notify.success(editing ? 'Tipo de mantenimiento actualizado' : 'Tipo de mantenimiento creado'); setFormOpen(false); void reload(); }}
      />
