...
2. Andá a **Alertas**. **Esperado:** ya hay **10 alertas pendientes**, generadas solas por el job automático: licencia de Lucía **vencida** y de Carlos **por vencer**; documentos por vencer (ART de Juan y María, licencia de Carlos) y vencido (licencia de Lucía); seguro de BBB222 **por vencer** y de CCC333 **vencido**; BBB222 con **km de mantenimiento superado**; CCC333 **inactivo**. Cada alerta es una tarjeta con ícono y color según su tipo y urgencia: rojo = vencida, ámbar = por vencer o requiere service, cian = informativa. Si apretás *Evaluar alertas*, el aviso emergente dice "no hubo cambios" (el job ya las creó al arrancar el backend). La pestaña **Resueltas** trae el historial: renovaciones de licencias y seguros, documentos reemplazados.
3. Marcá una alerta como resuelta (ícono de tilde) y confirmá. **Esperado:** desaparece de "Pendientes" y aparece en "Resueltas". (Si la condición sigue vigente, vuelve a aparecer en la próxima evaluación: la diaria de las 06:00, la del próximo arranque del backend o un nuevo *Evaluar alertas*. Resolver a mano no arregla la causa.)
4. **Auto-resolución:** si corregís la condición (ej. en Choferes, editá a Carlos y ponele una licencia con vencimiento lejano) y volvés a *Evaluar alertas*, esa alerta se **auto-resuelve** (el aviso dirá "... N resueltas automáticamente").
...
### 3.5 — Mejoras de UX (buscador, orden, carteles)

1. **Buscador de Viajes:** en **Viajes**, escribí "Pérez" en la caja con lupa. **Esperado:** sin apretar nada, a los instantes quedan solo los viajes de Juan Pérez. Probá con un destino ("Rosario"). No distingue mayúsculas ni tildes ("perez" también encuentra "Pérez"). La ✕ limpia la búsqueda.
2. **Mantenimiento — filtrar y ordenar:** en **Mantenimiento → Historial**, filtrá por un vehículo y después por un tipo. Cambiá *Ordenar por* a "Kilometraje" e invertí el orden con la flecha. También se puede ordenar tocando el encabezado de una columna. **Esperado:** la lista cambia en cada paso y la paginación sigue funcionando.
3. **Alertas — filtrar y ordenar:** en **Alertas**, elegí el atajo *Esta semana*, filtrá por tipo (agrupados por categoría) y por vehículo, y ordená por *Tipo de alerta*. **Esperado:** si nada coincide, un mensaje lo dice y ofrece *Limpiar filtros*.
4. **Carteles cuando algo no se puede:** en **Vehículos**, intentá desactivar el vehículo que está **En viaje**. **Esperado:** un cartel emergente abajo explica por qué no se puede. En **Choferes**, intentá dar de baja a Juan mientras tiene un viaje en curso: el motivo aparece dentro del mismo diálogo, que queda abierto. Las acciones que salen bien muestran un cartel verde de confirmación ("Viaje asignado", "Vehículo creado"…).
5. **Servidor caído:** con la app abierta, apagá el backend (Ctrl+C en la Terminal A) y tocá cualquier acción. **Esperado:** un cartel en castellano que dice que el servidor falló o no responde y sugiere reintentar, no un error genérico. (En desarrollo responde el proxy de Vite, así que el texto es el de error del servidor; sin internet dice "No se pudo conectar con el servidor…".) Volvé a levantarlo.
6. **Reportes sin período:** en **Reportes**, sin elegir fechas. **Esperado:** el botón está deshabilitado y un cartel dice qué falta elegir.
